...
import { TableContent , TableInfo } from '../Service/model-table';
...
export class MainOutputHtmlComponent implements OnInit {
  ...
  //   //let docc = doc.querySelector('tbody');
  //   //console.log(docc);
  //   let obj = [].map.call(doc.querySelectorAll('tr'), (tr) => {
  ...
  SocketIO(){
    ...
    //  // this.status = data ;
    // });
    //this.websocketService.emit('emit', 'Hello boy');
    this.websocketService.listen('TableInfo').subscribe((data)=>{
      this.TableInfo = data ; 
    });
    this.websocketService.listen('TableContent').subscribe((data)=>{
        this.TableContent = data ;
    });

    this.websocketService.listen('ErrorConnect').subscribe((data)=>{
      this.status = data ; 
    });
  }
  ...
  getSOTBMT(SOTBMT){
    ...
    this.TableContent = [];
    if(SOTBMT === ""){
      this.status = " Nhập Số Thông Báo Mời Thầu"
    }else{
      this.status = SOTBMT
      this.websocketService.emit('SOTBMT', SOTBMT);
  ...
  getPathUrl(pathUrl){
    ...
    this.TableContent = [];
    if(pathUrl === ""){
      this.status = " Nhập Đường dẫn website"
    }else{
      this.status = pathUrl
      this.websocketService.emit('pathUrl', pathUrl);
